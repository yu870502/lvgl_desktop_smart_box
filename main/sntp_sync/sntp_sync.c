...
void sync_run(void *arg)
{
    ...
    tzset();
    localtime_r(&now, &curTime);
    strftime(strftime_buf, sizeof(strftime_buf), "%c", &curTime);
...
static void obtain_time(void)
{
    ...
    for(;;)
    {
        if(esp_wifi_sta_get_ap_info(&ap_info) == ESP_OK) {
            ESP_LOGI(TAG, "wlan connected, start sntp sync");
            vTaskDelay(pdMS_TO_TICKS(1000)); // 精确延迟 5ms
            break;
        } else {
...
{
    xTaskCreate(sync_run, "sync run", 4096, NULL, 5, NULL);
    return 0;
}
