...
void _lv_disp_refr_timer(lv_timer_t * tmr)
{
    ...
    if(perf_label == NULL) {
        ...
        lv_obj_set_style_bg_opa(perf_label, LV_OPA_COVER, 0);
        lv_obj_set_style_bg_color(perf_label, lv_color_white(), 0);
        lv_obj_set_style_text_color(perf_label, lv_color_black(), 0);
    ...
    if(mem_label == NULL) {
        ...
        lv_obj_set_style_bg_opa(mem_label, LV_OPA_COVER, 0);
        lv_obj_set_style_bg_color(mem_label, lv_color_white(), 0);
        lv_obj_set_style_text_font(mem_label, &lv_font_montserrat_14, 0);    
        lv_obj_set_style_text_color(mem_label, lv_color_black(), 0);
    ...
    if(lv_tick_elaps(mem_monitor.mem_last_time) > 300) {
        ...
        lv_label_set_text_fmt(mem_label,
                              "%"LV_PRIu32 "kB used (%d %%)\n"
                              "%d%% frag.",
                              used_kb, mon.used_pct, mon.frag_pct);
    }
#endif
